...
func subsonicStar(c *gin.Context) {
	...
	for _, artistID := range artistIDs {
		...
		// First, check if artistID directly matches an artist name in songs
		var exists bool
		err := db.QueryRow("SELECT EXISTS(SELECT 1 FROM songs WHERE artist = ? AND cancelled = 0)", artistName).Scan(&exists)
		if err != nil {
		...
		if !exists {
			// Look up distinct artist names and compare generated IDs
			rows, err := db.Query("SELECT DISTINCT artist FROM songs WHERE cancelled = 0")
			if err != nil {
