...
WORKDIR /src
COPY --from=source_fetch_stage /src/AudioMuse-AI-MusicServer .
...
COPY --from=backend_build_stage /src/music-server-backend/music-server /app/audiomuse-server/music-server
# Copy the React frontend with its node_modules
COPY --from=frontend_build_stage /src/music-server-frontend /app/audiomuse-server/music-server-frontend
